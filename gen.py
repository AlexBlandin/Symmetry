...
for N, odd_N in [(N, N % 2) for N in range(MIN_N, MAX_N + 1)]:
  ...
  if odd_N:  # Odd N is solved, so we don't check and have no reason to break
    ...
    def scanline(
      diag, v, coord=[], adia=[]
    ):  # simple vertical constraints in TODO: adiag limits (so we're doing the full orbital constraints, can then check with nested constraints after but first do the simple case)
      return {
        Q
        ...
        and v[0] <= Q[1] <= v[1]
      }  # v[0]<=Q[1]<=v[1] reduced 15 dupes to 11

    for a in range(
      1 + middle[0], 2 * middle[0]
    ):  # This is correct and matches but since I'm simplifying I'll keep the checks to avoid breaking
    ...
    if ob_branches != dob_branches or sb_branches != dsb_branches:
      print(
        f"{N} doesn't match, (ob, sb) are {(len(dob_branches), len(dsb_branches))} != {(len(ob_branches), len(sb_branches))}"
      )
      break
else:
  print("All green.")
