...
for N, odd_N in [(N, N % 2) for N in range(MIN_N, MAX_N + 1)]:
  ...
  # Key Functions
  def legal(branch):
    return (len(branch) == numrc or (len(branch) == numrc - 1 and len(branch & intersection))) and all(
      (x, y) == (a, b) or (x != a and y != b and x + y != a + b and x - y != a - b)
      for (x, y), (a, b) in combinations(branch, 2)
    )
  ...
  def symmetries(squares):
    ...
    return {
      frozenset(squares),
      frozenset(mirrors(squares)),
      frozenset(mirrors(rotates(squares))),
      frozenset(mirrors(rotates(rotates(squares)))),
      frozenset(mirrors(rotates(rotates(rotates(squares))))),
      frozenset(rotates(squares)),
      frozenset(rotates(rotates(squares))),
      frozenset(rotates(rotates(rotates(squares)))),
    }
  ...
  def board(squares):
    return [
      "".join(
        ...
        if (x, y) in product(indices, middle) or (x, y) in product(middle, indices)
        else " "
        for x in indices
      )
      for y in indices
  ...
  if odd_N:  # Odd N is solved, so we don't check and have no reason to break
    ...
    def scanline(diag, coord=[], adia=[]):
      return {
        Q
        for Q in ((middle[0], diag - middle[0]), (middle[1], diag - middle[1]), (diag - middle[1], middle[1]))
        if sum(Q) == diag
        and 1 <= Q[0] <= N
        and 1 <= Q[1] <= N
        and Q[0] not in map(left, coord)
        and Q[1] not in map(right, coord)
        and Q[0] - Q[1] not in adia
      }

    for a in range(
      1 + middle[0], 2 * middle[0]
    ):  # This is correct and matches (also only generates the fundamentals as expected which is great) but since I'm simplifying I'll keep the checks to avoid breaking
            ...
            for rc3 in scanline(c, [rc1, rc2], [adg, rc2[0] - rc2[1]]):
              if rc2 in intersection or rc3 in intersection:
                ...
                if legal(frozenset(branch)):
                  if frozenset(branch) in ob_branches:
                    ...  # printout([branch, lexo(branch)],True,True)
                  else:
                    include(branch)
              else:
  ...
  if ob != expected:
    err.append(f"expected {expected} ob")
  expected = (
    (N - 1) * (N - 3) // 8 + 1 if odd_N else (N**4 - 22 * N**3 + 202 * N**2 - 888 * N + 1584) // 8 if N >= 8 else sb
  )
  if sb != expected:
    err.append(f"expected {expected} sb")
  if sb != len(sb_branches):
...
# Write out results
open("./data/results.txt", mode="w").write(tabulate(table, headers="firstrow", floatfmt=["d", "d", "d", ".3f"]))
